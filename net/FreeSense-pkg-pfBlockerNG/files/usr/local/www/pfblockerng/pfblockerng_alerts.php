...
if (!$alert_summary) {
	...
	$btnsubmit->removeClass('btn-primary')->addClass('btn-primary btn-sm');
	...
	$btnclear->removeClass('btn-primary')->addClass('btn-danger btn-sm');
...
<div class="card mb-3" style="display: inline-block; width: 100%;">
	<div class="card-header">
		<h2 class="h5 mb-0">&nbsp;<?=gettext('Unlocked IP(s) & Domain(s)')?></h2>
	</div>
...
<form action="/pfblockerng/pfblockerng_alerts.php" method="post" name="iform_stats" id="iform_stats" class="">
...
<div class="card mb-3" id="Alert_Stats_<?=$stat_type?>" style="display: inline-block; width: 100%;">
	<div class="card-header">
		<h2 class="h5 mb-0">
			<? if ($alertrefresh == 'on'): ?>
			<i class="fa-solid fa-pause-circle" id="PauseRefresh" " title="Pause Alerts Refresh"></i>&nbsp;
			...
			<span class="widget-heading-icon float-end">
				<a data-bs-toggle="collapse" href="#Alert_Stats_<?=$stat_type?>_panel-body" id="Alert_Stats_A_<?=$stat_type?>">
		...
		<div class="card-body collapse <?=$collapse_status?>" id="Alert_Stats_<?=$stat_type?>_panel-body" style="overflow-x: auto;">
			...
			<div id="chart" class="d3-chart" style="overflow: hidden;">
				...
				<div class="btn-group float-end" style="margin-right: 10px;">
					<a href="#" class="btn btn-sm btn-secondary dropdown-toggle" data-bs-toggle="dropdown" role="button" aria-expanded="false">
						Date Range
					</a>
					<ul class="dropdown-menu" role="menu" style="padding: 1px 1px 1px 1px; font-size: smaller;">
						...
						<li id="chartEvent" value="<?=$event?>">
							<a href="#" class="dropdown-item navlnk"><?=$type?></a>
						</li>
						<?php endforeach;?>
					</ul>
				</div>
...
function ip_suppression_type() {
	...
	$("div[role=dialog]").find('button').addClass('btn-info btn-sm');
}
...
events.push(function() {
	...
	$('[id=chartEvent]').click(function() {
		if (((!isNaN(parseFloat($(this).attr('value'))) && isFinite($(this).attr('value')))) && $(this).attr('value') > 0 && $(this).attr('value') < 8065) {

...
require_once('/usr/local/pkg/pfblockerng/pfblockerng_extra.inc');	// 'include functions' not yet merged into FreeSense
...
function pfb_update_check($header, $list_url, $pfbfolder, $pfborig, $pflex, $format, $vtype, $srcint=FALSE) {
	...
	if (file_exists($local_file)) {
		...
		// Determine if URL is a FreeSense localfile
		$localfile = FALSE;
...
// Collect all FreeSense 'Port' Aliases
...
$section->addInput(new Form_StaticText(
	...
	. 'FreeSense by default implicitly blocks all unsolicited inbound traffic to the WAN interface.<br />'
...
$action_txt = "Default: <strong>Disabled</strong>
		...
		<div class=\"infoblock alert-info clearfix\">
			...
			This enables a pfBlockerNG list to be used by name, in any firewall rule or FreeSense function, as desired.
				<ul>
...
function pfb_build_reputation_tab($et_options='') {

 ...
 * FreeSense from the following contributors:
 *
 * pkg_mgr_install.php
 * Part of FreeSense (https://www.freesense.org)
...
$btn_run->removeClass('btn-primary')->addClass('btn-primary btn-sm')->setWidth(1);
...
$btn_logview->removeClass('btn-primary')->addClass('btn-primary btn-sm')->setWidth(1)
...
if ($pfb['enable'] == 'on' && isset($pconfig['run']) && !empty($pconfig['pfb_force'])) {
	...
	if ($pfb_wizard) {
		...
		$wizard_log =
'<div class="float-start alert alert-info clearfix" style="width: 100%;" role="alert">
	...
	<ul>
		...
		<li>For DNSBL, ensure that all of your LAN devices are pointed at FreeSense ONLY for DNS resolution.</li>
...
events.push(function(){
	...
	$('label[class="col-sm-2 col-form-label"]:eq(6)').remove();

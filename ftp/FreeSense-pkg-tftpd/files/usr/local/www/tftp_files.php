...
if ($_POST['upload'] == "Upload" && $_FILES["tftpd_fileup"]["error"] == UPLOAD_ERR_OK) {
	if (is_uploaded_file($_FILES['tftpd_fileup']['tmp_name'])) {
		...
		if (!preg_match('/^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/', $name) ||
		    (int)$_FILES["tftpd_fileup"]["size"] > 64 * 1024 * 1024) {
			$input_errors[] = gettext('The upload filename or size is invalid.');
		} elseif (!move_uploaded_file($tmp_name, FILES_DIR . "/{$name}")) {
			$input_errors[] = gettext('Failed to store the uploaded file.');
		} else {
			chmod(FILES_DIR . "/{$name}", 0640);
...
<div class="card mb-3">
	<div class="card-header"><h2 class="h5 mb-0"><?=gettext("TFTP Server Files Management")?></h2></div>
	...
	<nav class="action-buttons">
		<button data-bs-toggle="modal" data-bs-target="#uploader" role="button" aria-expanded="false" type="button" name="tftpd_upload" id="tftpd_upload" class="btn btn-info btn-sm" title="<?=gettext('Upload files');?>">
